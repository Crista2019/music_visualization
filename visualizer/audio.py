    ...
    def on_update(self):
        ...
        if self.generator and num_frames != 0:
            ...
            assert len(data) == num_frames * self.num_channels, \
                "asked for (%d * %d) frames but got %d" % (num_frames,
...
def get_audio_devices():
    ...
    out_devs = [{'index': 'None', 'name': 'Default',
                 'channels': 0, 'latency': (0, 0)}]
    in_devs = [{'index': 'None', 'name': 'Default',
                'channels': 0, 'latency': (0, 0)}]
...
def print_audio_devices():
    ...
    for d in devs['output']:
        print(
            '{index:>5}: {name:<40} {channels:<6} {latency[0]:.3f} - {latency[1]:.3f}'.format(**d))
